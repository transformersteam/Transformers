...
{
    public class CompanyIntegralController : Controller
    {
        ...
        public string GetAll(string request, string actionName, object obj = null)
        {
            ...
            hc.BaseAddress = new Uri("http://localhost:8888/api/CompanyIntegral/");
            //创建取服务端回包的任务
        ...
        {
            var list = GetAll("get", "GetCompanyIntegralList");
            List<CompanyIntegral> echartvalue = JsonConvert.DeserializeObject<List<CompanyIntegral>>(list);
            test t = new test();
